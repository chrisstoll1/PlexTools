...
function buildNotifications(response){
...
function getStatus(taskID, calID) {
    console.log(queue);
    $.ajax({
        url: `/tasks/${taskID}/`,
        method: 'GET'
        ...
        if (taskStatus === 'SUCCESS'){
            console.log(taskStatus, taskID, calID);
            data.calendars.forEach(calendar => {
                if (calendar.calID == calID){
                    calendar.data = res.task_result;
                }
            });
            queue = queue.filter(e => e !== taskID);
        }else if (taskStatus === 'FAILURE'){
            // resolve(false);
        }else{
            setTimeout(function() {
                getStatus(res.task_id, calID);
            }, 1000);
        };
        })
        .fail((err) => {
...
function loadNotifications(){
    ...
    $.ajax(settings).done(function (response) {
        data = response;
        data.calendars.forEach(calendar => {
            getStatus(calendar.data.task_id, calendar.calID);
            queue.push(calendar.data.task_id);
        });
        monitorQueue();
    });  
}
